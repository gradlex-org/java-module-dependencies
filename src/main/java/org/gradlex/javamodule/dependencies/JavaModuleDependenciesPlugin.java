...
public abstract class JavaModuleDependenciesPlugin implements Plugin<ExtensionAware> {
    ...
    private static final String REGISTER_HELP_TASKS_PROPERTY =
    ...
    private void applyProject(Project project) {
        ...
        project.getPlugins()
                .withType(JavaPlugin.class, javaPlugin -> setupForJavaProject(project, javaModuleDependencies));
    }
    ...
    private void setupForJavaProject(Project project, JavaModuleDependenciesExtension javaModuleDependencies) {
        ...
        TaskProvider<Task> checkAllModuleInfo = registerHelpTasks
                ? project.getTasks().register("checkAllModuleInfo", t -> {
        ...
        project.getPlugins()
                .withId(
                        ...
                        analysisPlugin -> DependencyAnalysisBridge.registerDependencyAnalysisPostProcessingTask(
                                project, checkAllModuleInfo));
    }

    private void setupExtraJavaModulePluginBridge(
            Project project, JavaModuleDependenciesExtension javaModuleDependencies) {
        project.getPlugins()
                .withId(
                        EXTRA_JAVA_MODULE_INFO_PLUGIN_ID,
                        e -> ExtraJavaModuleInfoBridge.autoRegisterPatchedModuleMappings(
                                project, javaModuleDependencies));
    }
    ...
    private void setupDirectivesDSL(Project project, JavaModuleDependenciesExtension javaModuleDependencies) {
        ...
        sourceSets.all(sourceSet -> {
            ...
            if (moduleInfoFile.exists()) {
                extensions.create(
                        GradleOnlyDirectives.class,
                        extensionName,
                        GradleOnlyDirectivesInternal.class,
                        sourceSet,
                        mainSourceSet,
                        javaModuleDependencies);
            } else {
    ...
    private void setupModuleDependenciesTask(Project project) {
        SourceSetContainer sourceSets = project.getExtensions().getByType(SourceSetContainer.class);
        TaskProvider<ModuleDependencyReport> moduleDependencies = project.getTasks()
                .register("moduleDependencies", ModuleDependencyReport.class, t -> t.setGroup(HELP_GROUP));
    ...
    private void setupReportTasks(Project project) {
        ...
        project.getTasks().register("analyzeModulePath", ModulePathAnalysis.class, t -> {
            ...
            for (SourceSet sourceSet : sourceSets) {
                t.getClasspathConfigurations()
                        .add(project.getConfigurations().getByName(sourceSet.getCompileClasspathConfigurationName()));
                t.getClasspathConfigurations()
                        .add(project.getConfigurations().getByName(sourceSet.getRuntimeClasspathConfigurationName()));
            }
        });
    ...
    private void setupMigrationTasks(Project project, JavaModuleDependenciesExtension javaModuleDependencies) {
        ...
        sourceSets.all(sourceSet -> {
            TaskProvider<ModuleInfoGenerate> generateModuleInfo = project.getTasks()
                    .register(sourceSet.getTaskName("generate", "ModuleInfoFile"), ModuleInfoGenerate.class, t -> {
                        t.setGroup("java modules");
                        ...
                        t.getModuleName()
                                .convention(project.provider(() -> project.getGroup() + "."
                                        + sourceSetToModuleName(project.getName(), sourceSet.getName())));

                        t.getApiDependencies()
                                .convention(declaredDependencies(project, sourceSet.getApiConfigurationName()));
                        t.getImplementationDependencies()
                                .convention(
                                        declaredDependencies(project, sourceSet.getImplementationConfigurationName()));
                        t.getCompileOnlyApiDependencies()
                                .convention(
                                        declaredDependencies(project, sourceSet.getCompileOnlyApiConfigurationName()));
                        t.getCompileOnlyDependencies()
                                .convention(declaredDependencies(project, sourceSet.getCompileOnlyConfigurationName()));
                        t.getRuntimeOnlyDependencies()
                                .convention(declaredDependencies(project, sourceSet.getRuntimeOnlyConfigurationName()));

                        t.getModuleInfoFile()
                                .convention(project.getLayout()
                                        .file(project.provider(() -> new File(
                                                sourceSet
                                                        .getJava()
                                                        .getSrcDirs()
                                                        .iterator()
        ...
        project.getTasks().register("generateBuildFileDependencies", BuildFileDependenciesGenerate.class, t -> {
            ...
            sourceSets.all(sourceSet -> t.addDependencies(
                    sourceSet.getName(),
                    collectDependencies(
                            project,
                            javaModuleDependencies,
    ...
    private void setupOrderingCheckTasks(
            Project project,
        ...
        sourceSets.configureEach(sourceSet -> {
            TaskProvider<ModuleDirectivesOrderingCheck> checkModuleInfo = project.getTasks()
                    .register(sourceSet.getTaskName("check", "ModuleInfo"), ModuleDirectivesOrderingCheck.class, t -> {
                        t.setGroup("java modules");
                        t.setDescription("Check order of directives in 'module-info.java' in '" + sourceSet.getName()
                                + "' source set");

                        ModuleInfo moduleInfo = javaModuleDependencies
                                .getModuleInfoCache()
                                .get()
                                .get(sourceSet, project.getProviders());
                        File folder = javaModuleDependencies
                                .getModuleInfoCache()
                                .get()
                                .getFolder(sourceSet, project.getProviders());
                        if (folder != null) {
                            t.getModuleInfoPath().convention(new File(folder, "module-info.java").getAbsolutePath());
                        }
                        t.getModuleNamePrefix()
                                .convention(moduleInfo.moduleNamePrefix(project.getName(), sourceSet.getName(), false));
                        t.getModuleInfo().convention(moduleInfo);

                        t.getReport()
                                .convention(project.getLayout()
                                        .getBuildDirectory()
                                        .file("reports/module-info-analysis/" + sourceSet.getName() + "-ordering.txt"));
                    });
    ...
    private void process(
            ModuleInfo.Directive moduleDirective,
            String gradleConfiguration,
            SourceSet sourceSet,
            Project project,
            JavaModuleDependenciesExtension javaModuleDependenciesExtension) {
        Configuration conf = project.getConfigurations().findByName(gradleConfiguration);
        if (conf != null) {
            conf.withDependencies(
                    d -> readModuleInfo(moduleDirective, sourceSet, project, conf, javaModuleDependenciesExtension));
        } else {
            project.getConfigurations().whenObjectAdded(lateAddedConf -> {
                if (gradleConfiguration.equals(lateAddedConf.getName())) {
                    lateAddedConf.withDependencies(d -> readModuleInfo(
                            moduleDirective, sourceSet, project, lateAddedConf, javaModuleDependenciesExtension));
                }
            });
        }
    }
    ...
    private void readModuleInfo(
        ...
        ModuleInfo moduleInfo =
                javaModuleDependenciesExtension.getModuleInfoCache().get().get(sourceSet, project.getProviders());
    ...
    private void declareDependency(
            String moduleName,
            Project project,
            SourceSet sourceSet,
            Configuration configuration,
        ...
        project.getDependencies()
                .addProvider(configuration.getName(), javaModuleDependencies.create(moduleName, sourceSet));
    }

    private List<BuildFileDependenciesGenerate.DependencyDeclaration> collectDependencies(
            Project project,
            JavaModuleDependenciesExtension javaModuleDependencies,
            SourceSet sourceSet,
            ModuleInfo.Directive directive,
            String scope) {
        ModuleInfo moduleInfo =
                javaModuleDependencies.getModuleInfoCache().get().get(sourceSet, project.getProviders());
        if (moduleInfo == ModuleInfo.EMPTY) {
            // check if there is a whiltebox module-info we can use isntead
            File sourceSetDir =
                    sourceSet.getJava().getSrcDirs().iterator().next().getParentFile();
            File whiteboxModuleInfoFile = new File(sourceSetDir, "java9/module-info.java");
            if (whiteboxModuleInfoFile.exists()) {
                moduleInfo = new ModuleInfo(project.getProviders()
                        .fileContents(project.getLayout()
                                .getProjectDirectory()
                                .file(whiteboxModuleInfoFile.getAbsolutePath()))
                        .getAsText()
                        .get());
            }
        }
        return moduleInfo.get(directive).stream()
                .map(moduleName -> new BuildFileDependenciesGenerate.DependencyDeclaration(
                        scope, moduleName, javaModuleDependencies.ga(moduleName)))
